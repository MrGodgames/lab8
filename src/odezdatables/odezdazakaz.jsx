...
function OdezdaZakaz() {
  ...
  useEffect(() => {
    let active = true
    fetch(`${API_BASE}/api/odezda/zakaz`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/odezda/zakaz`, {
        method: 'POST',
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE}/api/odezda/zakaz/${id}`, {
        method: 'DELETE',
      })
