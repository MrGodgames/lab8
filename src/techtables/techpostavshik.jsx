...
function TechPostavshik() {
  ...
  useEffect(() => {
    let active = true
    fetch(`${API_BASE}/api/tech/postavshik`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/tech/postavshik`, {
        method: 'POST',
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE}/api/tech/postavshik/${id}`, {
        method: 'DELETE',
      })
